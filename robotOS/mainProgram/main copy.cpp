...
int main()
{
	...
	while (true)
	{
		...
		case TESTINGMOTORLEFT:
			...
			break;
		case EMERGENCY_STOP:
			printf("[info] Emergency stop made!\n");
			pct_pwm = 0;
			set_motor_speed(0, 0);
			EVENT = NOTHING;
			STATUS = STANDBY;
...
void line_detection()
{
	...
	while (true)
	{
		...
		erode(whiteLine, whiteLine, Mat(), Point(-1, -1), 10, 1, 1);
		dilate(whiteLine, whiteLine, Mat(), Point(-1, -1), 10, 1, 1);
		...
		if (contours.size() > 0)
		{
			...
			putText(frame, "angle:" + to_string(camera_angle), Point(10, 40), FONT_HERSHEY_SIMPLEX, 1, Scalar(255, 255, 255), 2);
			putText(frame, "offset:" + to_string(camera_offset), Point(10, 80), FONT_HERSHEY_SIMPLEX, 1, Scalar(255, 255, 255), 2);
			// draw a line in the middel of the line.
			line(frame, Point(centerline, 100), Point(centerline, 150), Scalar(255, 255, 255), 3);
...
void drive()
{
	int steering_offset = 80.0 / ((screen_width / 2) + 90) * (camera_offset + camera_angle);
...
void set_motor_speed(int pct_pwm_left, int pct_pwm_right)
{
	...
	if (motor_balance > 0)
	{
		pwm_right = double(1024.0 / 100 * pct_pwm_right) / 100 * (100 + motor_balance);
		pwm_left = 1024.0 / 100 * pct_pwm_left;
	}
	else if (motor_balance < 0)
	{
		pwm_right = 1024.0 / 100 * pct_pwm_right;
		pwm_left = double(1024.0 / 100 * pct_pwm_left) / 100 * (100 - motor_balance);
	}
